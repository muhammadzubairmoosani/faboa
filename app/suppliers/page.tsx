...
import { supplier } from "../utils/data.json";

import { useState } from "react";
import {
  Header,
  Sidebar,
  SuppliersFiltersAndSearch,
  SuppliersTable
} from "../components";
...
export default function MembersPage() {
  ...
  const currentMembers: any = supplier.slice(
    startIndex,
    startIndex + ITEMS_PER_PAGE
  );

  const totalPages = Math.ceil(supplier.length / ITEMS_PER_PAGE);

  const handleSelectAll = () => {
    if (selectedMembers.length === currentMembers.length) {
      setSelectedMembers([]);
    } else {
      setSelectedMembers(currentMembers.map((member: Supplier) => member.id));
    }
  };
